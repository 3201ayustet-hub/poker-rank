(function(){
  ...
  const seed={players:[],sessions:[],games:[],settings:{roomName:'PRIVATE POKER ROOM',version:'1.3'}};
  const clone=v=>JSON.parse(JSON.stringify(v));
  const isUuid=v=>/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(v||''));
  const uid=()=>crypto.randomUUID();
  function normalize(data){return {players:Array.isArray(data?.players)?data.players:[],sessions:Array.isArray(data?.sessions)?data.sessions:[],games:Array.isArray(data?.games)?data.games:[],settings:{...clone(seed.settings),...(data?.settings||{}),version:'1.3'}}}
