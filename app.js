(function () {
  ...
  function menuModal(){ modal(`<div class="modal-head"><div><div class="eyebrow">SYSTEM · v1.6</div><h2>DATA BOOK</h2></div><button class="icon-btn" data-action="close-modal">×</button></div><div class="menu-stack"><button class="quiet-action wide" data-action="export-data">EXPORT BACKUP</button><label class="quiet-action wide file-label">IMPORT BACKUP<input id="import-file" type="file" accept="application/json"></label><button class="danger-action wide" data-action="reset-data">RESET ALL DATA</button></div><div class="cloud-panel"><span class="cloud-dot ${cloudStatus}"></span><div><b>SUPABASE SYNC</b><small>${cloudStatus.toUpperCase()}</small></div></div><p class="muted copy">Supabaseと同期し、同じURLを開いた複数端末へ変更を反映します。通信できない場合も端末内に保存されます。</p>`); }
