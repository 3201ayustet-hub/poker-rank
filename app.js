(function () {
  ...
  function save(){ window.PokerStorage.save(db); }
  ...
  function homeView(){
    ...
    const avg=st.active.length?st.active.reduce((sum,pid)=>sum+currentStack(g,pid),0)/st.active.length:0;
      ...
      <main class="page home-page">
        ...
        <section class="ledger-section standings-section">
          ...
          <div class="standings-list">
            ${st.active.map((pid,i)=>`<div class="standing-row ${i===0?'is-leader':''}"><span class="standing-rank">${String(i+1).padStart(2,'0')}</span><strong>${esc(playerById(pid)?.name||'—')}</strong><b>${fmt.format(currentStack(g,pid))}</b></div>`).join('')}
            ${st.out.length?`<div class="out-divider"><span>OUT</span></div>${st.out.map(pid=>`<div class="standing-row is-out"><span class="standing-rank">${g.results[pid].rank?String(g.results[pid].rank).padStart(2,'0'):'—'}</span><strong>${esc(playerById(pid)?.name||'—')}</strong><b>OUT</b></div>`).join('')}`:''}
          </div>
        </section>
  ...
  function playersView(){
    ...
    return `${topBar('MEMBERS')}<main class="page">
      <section class="page-intro compact-intro"><div class="eyebrow">PRIVATE MEMBERS CLUB</div><h1>Players</h1><p>最大10名。マイハンドとトーナメント戦績を記録します。</p></section>
      <section class="ledger-section">${sectionHead('MEMBER DIRECTORY',`${list.length} / 10`)}
      ${list.map(p=>{const s=playerStats(p.id);return `<button class="member-row" data-nav="player" data-player-id="${p.id}"><span class="member-no">${String(p.memberNo||0).padStart(2,'0')}</span><span class="member-main"><strong>${esc(p.name)}</strong><small>MY HAND · ${esc(p.myHand||'—')}</small></span><span class="member-perf">${s.games?s.performance.toFixed(1)+'%':'—'}</span></button>`}).join('')||'<p class="muted copy">プレイヤーを登録してください。</p>'}
  ...
  function playerView(pid){
    ...
    return `${topBar(p.name,'back-players')}<main class="page">
      <section class="member-card"><div class="eyebrow">MEMBER ${String(p.memberNo||0).padStart(2,'0')}</div><div class="member-card-name">${esc(p.name)}</div><div class="my-hand"><span>MY HAND</span><strong>${esc(p.myHand||'—')}</strong></div></section>
  ...
  function sessionsView(){
    ...
    return `${topBar('SESSIONS')}<main class="page"><section class="page-intro compact-intro"><div class="eyebrow">TOURNAMENT DAYS</div><h1>Sessions</h1><p>1回の集まりの中で、複数トーナメントを管理します。</p></section><button class="primary-action wide" data-action="new-session">＋ NEW SESSION</button><section class="ledger-section">${sectionHead('SESSION LEDGER',`${list.length} SESSIONS`)}${list.map(s=>{const gs=db.games.filter(g=>g.sessionId===s.id);const live=gs.some(g=>g.status==='live');return `<button class="ledger-row tall" data-nav="session" data-session-id="${s.id}"><span class="ledger-index">${String(s.sessionNumber||0).padStart(2,'0')}</span><span class="ledger-main"><strong>${formatDate(s.date)}</strong><small>${esc(s.name||'PRIVATE GAME')} · ${gs.length} GAMES</small></span><span class="session-status ${live?'live-text':''}">${live?'● LIVE':s.status==='open'?'OPEN':'CLOSED'}</span></button>`}).join('')||'<p class="muted copy">まだSessionがありません。</p>'}</section></main>${bottomNav('session-list')}`;
  }
  ...
  function graphData(g){
    ...
    const state={}; const active=new Set();
    ...
    nodes.forEach(n=>{
      if(n.type==='break'){
        Object.entries(n.break.stacks||{}).forEach(([pid,v])=>{
          if(!active.has(pid))return; state[pid]=Number(v)||0;
          const seg=segments[pid][segments[pid].length-1]; seg.push({t:n.at,v:state[pid]});
        });
      } else if(n.type==='out'||n.type==='elimination'){
        const pid=n.event.playerId; if(pid&&active.has(pid)){
          const seg=segments[pid][segments[pid].length-1];
          const before=Number(n.event.meta?.stackBefore); if(Number.isFinite(before)&&before>=0&&seg[seg.length-1]?.t!==n.at)seg.push({t:n.at,v:before});
          seg.push({t:n.at,v:0}); active.delete(pid); delete state[pid];
        }
      } else if(n.type==='reentry'){
        const pid=n.event.playerId; if(pid){ active.add(pid); state[pid]=g.startingStack; segments[pid].push([{t:n.at,v:g.startingStack}]); }
      }
      if(active.size){ average.push({t:n.at,v:[...active].reduce((s,pid)=>s+(state[pid]||0),0)/active.size}); }
    });
  ...
  function renderStackChart(g,{home=false}={}){
    ...
    return `<div class="stack-chart-wrap"><svg class="stack-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="プレイヤー別スタック推移"><g>${grid}${breakMarks}${lines}${avgPath}${xt}</g></svg><div class="chart-legend">${legend}</div><div class="chart-note">横軸はゲーム開始からの実時間。OUTで線が切れ、RE-ENTRYは開始スタックから再開します。</div></div>`;
  }
  ...
  function recordsView(){
    ...
    const perf=ranked[0]; const games=[...ranked].sort((a,b)=>b.s.games-a.s.games)[0]; const re=[...ranked].sort((a,b)=>b.s.reentries-a.s.reentries)[0];
    ...
    return `${topBar('ROOM RECORDS')}<main class="page records-page">
      <section class="hall-hero"><div class="eyebrow">HALL OF RECORDS</div><div class="hall-kicker">ALL-TIME TITLE LEADER</div><div class="hall-number">${titles?.s.titles??'—'}</div><div class="hall-name">${esc(titles?.p.name||'THE BOOK IS OPEN')}</div><div class="hall-sub">${titles?`${titles.s.games} GAMES · ${titles.s.winRate.toFixed(1)}% WIN RATE`:'Play tournaments to build the room history.'}</div></section>
      <section class="record-ledger">${sectionHead('ROOM RECORDS','ALL TIME')}
        ${recordEntry('01','MOST TITLES',titles?.s.titles,titles?.p.name,'TITLE HOLDER')}
        ${recordEntry('02','BEST PERFORMANCE',perf?perf.s.performance.toFixed(1)+'%':'—',perf?.p.name,'FIELD MASTER')}
        ${recordEntry('03','MOST GAMES',games?.s.games,games?.p.name,'IRON PLAYER')}
        ${recordEntry('04','MOST RE-ENTRIES',re?.s.reentries,re?.p.name,'RE-ENTRY KING','wine')}
      </section>
      <section class="ledger-section performance-ranking">${sectionHead('PLAYER PERFORMANCE','ALL-TIME STANDINGS')}${ranked.map((x,i)=>`<div class="rank-card ${i===0?'rank-one':''}"><div class="rank-main"><span>${String(i+1).padStart(2,'0')}</span><strong>${esc(x.p.name)}</strong><b>${x.s.performance.toFixed(1)}%</b></div><div class="rank-meta"><span>${x.s.titles} TITLES</span><span>${x.s.games} GAMES</span><span>${x.s.winRate.toFixed(1)}% WIN</span><span>${x.s.reentryAvg.toFixed(2)} RE/G</span></div></div>`).join('')||'<p class="muted copy">戦績が蓄積されるとランキングが表示されます。</p>'}</section>
      <section class="ledger-section additional-records">${sectionHead('ADDITIONAL RECORDS')}
        <div class="mini-record"><span>LONGEST WIN STREAK</span><b>${streak?.streak||0}</b><small>${esc(streak?.p.name||'—')}</small></div>
        <div class="mini-record"><span>CLEANEST PLAYER</span><b>${clean?clean.s.reentryAvg.toFixed(2):'—'}</b><small>${esc(clean?.p.name||'—')} · RE/G</small></div>
      </section>
      <section class="room-history">${sectionHead('ROOM HISTORY')}
        <div class="history-stats"><span><b>${completedGames().length}</b><small>TOURNAMENTS</small></span><span><b>${players().length}</b><small>MEMBERS</small></span><span><b>${db.games.reduce((n,g)=>n+totalReentries(g),0)}</b><small>RE-ENTRIES</small></span></div>
        <div class="first-game"><small>FIRST RECORDED GAME</small><strong>${first?formatDate(sessionById(first.sessionId)?.date):'—'}</strong></div>
      </section>
      </main>${bottomNav('records')}`;
  }
  function recordEntry(no,label,value,name,badge,tone=''){ return `<div class="record-entry ${tone}"><span class="record-no">${no}</span><div><div class="record-label">${label}</div><div class="record-value">${value??'—'}</div><div class="record-holder">${esc(name||'—')}</div></div><em>${badge}</em></div>`; }
  ...
  function addPlayerModal(existing){ modal(`<div class="modal-head"><div><div class="eyebrow">MEMBER RECORD</div><h2>${existing?'EDIT MEMBER':'NEW MEMBER'}</h2></div><button class="icon-btn" data-action="close-modal">×</button></div><form id="player-form" ${existing?`data-player-id="${existing.id}"`:''}><label class="form-field"><span>PLAYER NAME</span><input name="name" required maxlength="24" value="${esc(existing?.name||'')}"></label><label class="form-field"><span>MY HAND</span><input name="myHand" maxlength="8" placeholder="AKs" value="${esc(existing?.myHand||'')}"></label><button class="primary-action wide" type="submit">${existing?'SAVE CHANGES':'REGISTER MEMBER'}</button></form>`); }
  ...
  function menuModal(){ modal(`<div class="modal-head"><div><div class="eyebrow">SYSTEM · v1.2</div><h2>DATA BOOK</h2></div><button class="icon-btn" data-action="close-modal">×</button></div><div class="menu-stack"><button class="quiet-action wide" data-action="export-data">EXPORT BACKUP</button><label class="quiet-action wide file-label">IMPORT BACKUP<input id="import-file" type="file" accept="application/json"></label><button class="danger-action wide" data-action="reset-data">RESET ALL DATA</button></div><p class="muted copy">現在はこの端末のブラウザ内に保存されます。</p>`); }
  ...
  function markOut(gid,pid){ const g=gameById(gid),p=playerById(pid); if(!g||!p)return; ensureGameShape(g); const before=currentStack(g,pid); if(!confirm(`${p.name} をOUT登録しますか？`))return; g.results[pid].isOut=true; g.currentStacks[pid]=0; if(!g.reentryOpen){const rank=livePlayers(g).length+1;g.results[pid].rank=rank;addEvent(g,'elimination',pid,`${p.name} — ${ordinal(rank)}`,{stackBefore:before});}else addEvent(g,'out',pid,`${p.name} OUT`,{stackBefore:before}); save(); render(); }
  ...
  function exportBackup(){ const blob=new Blob([window.PokerStorage.exportData()],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`poker-rank-v1.2-backup-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000); }
  ...
  function handleClick(e){
    ...
    else if(a==='mark-out')markOut(el.dataset.gameId,el.dataset.playerId); else if(a==='reenter')reenter(el.dataset.gameId,el.dataset.playerId); else if(a==='close-reentry')closeReentry(el.dataset.gameId); else if(a==='finish-game')finishGame(el.dataset.gameId);
  ...
  function handleSubmit(e){
    if(e.target.id==='player-form'){e.preventDefault();const fd=new FormData(e.target),pid=e.target.dataset.playerId;const name=String(fd.get('name')||'').trim();if(!name)return;if(pid){const p=playerById(pid);p.name=name;p.myHand=String(fd.get('myHand')||'').trim();}else{if(players().length>=10){alert('プレイヤーは最大10名です。');return;}db.players.push({id:id('pl'),memberNo:Math.max(0,...db.players.map(p=>p.memberNo||0))+1,name,myHand:String(fd.get('myHand')||'').trim(),createdAt:new Date().toISOString()});}save();closeModal();navigate('players');}
  ...
  function handleChange(e){if(e.target.id==='import-file'&&e.target.files[0]){const reader=new FileReader();reader.onload=()=>{try{db=window.PokerStorage.importData(reader.result);db.games.forEach(ensureGameShape);closeModal();navigate('home');}catch(err){alert(err.message)}};reader.readAsText(e.target.files[0]);}}
