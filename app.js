(function () {
  ...
  function recordsView(){
    ...
    return `${topBar('ROOM RECORDS')}<main class="page records-page simple-records">
      ...
      <section class="honor-carousel" aria-label="主要ルーム記録">${honors.map((h,i)=>`<button type="button" data-action="focus-honor" data-honor-index="${i}" class="honor-slide ${h.red?'red-honor':''} ${activeHonorIndex===i?'is-active':''}" aria-pressed="${activeHonorIndex===i?'true':'false'}"><div class="honor-corner">${h.corner}</div><div class="honor-slide-body"><div class="honor-label">${h.label}</div><div class="honor-value">${h.value}</div><div class="honor-divider"></div><div class="honor-holder">${esc(h.name)}</div><div class="honor-date">${h.date}</div></div><div class="honor-tag">${h.tag}</div><div class="honor-corner bottom">${h.corner}</div></button>`).join('')}</section>
      <div class="carousel-dots">${honors.map((_,i)=>`<button data-action="focus-honor" data-honor-index="${i}" class="${i===activeHonorIndex?'active':''}" aria-label="Honor ${i+1}"></button>`).join('')}</div>
      ...
      <p class="honor-hint">ⓘ 横にスワイプしてカードを閲覧し、タップで詳細を表示します</p>
  ...
  function menuModal(){ modal(`<div class="modal-head"><div><div class="eyebrow">SYSTEM · v1.5</div><h2>DATA BOOK</h2></div><button class="icon-btn" data-action="close-modal">×</button></div><div class="menu-stack"><button class="quiet-action wide" data-action="export-data">EXPORT BACKUP</button><label class="quiet-action wide file-label">IMPORT BACKUP<input id="import-file" type="file" accept="application/json"></label><button class="danger-action wide" data-action="reset-data">RESET ALL DATA</button></div><div class="cloud-panel"><span class="cloud-dot ${cloudStatus}"></span><div><b>SUPABASE SYNC</b><small>${cloudStatus.toUpperCase()}</small></div></div><p class="muted copy">Supabaseと同期し、同じURLを開いた複数端末へ変更を反映します。通信できない場合も端末内に保存されます。</p>`); }
  ...
  function exportBackup(){ const blob=new Blob([window.PokerStorage.exportData()],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`poker-rank-v1.5-backup-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000); }
  ...
  function handleClick(e){
    ...
    else if(a==='mark-out')markOut(el.dataset.gameId,el.dataset.playerId); else if(a==='confirm-out')confirmOut(el.dataset.gameId,el.dataset.playerId); else if(a==='focus-honor'){activeHonorIndex=Number(el.dataset.honorIndex)||0;render();requestAnimationFrame(()=>document.querySelector(`.honor-slide[data-honor-index="${activeHonorIndex}"]`)?.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'}));} else if(a==='reenter')reenter(el.dataset.gameId,el.dataset.playerId); else if(a==='close-reentry')closeReentry(el.dataset.gameId); else if(a==='finish-game')finishGame(el.dataset.gameId);
