(function () {
  ...
  function recordsView(){
    ...
    return `${topBar('ROOM RECORDS')}<main class="page records-page">
      ...
      <section class="honor-deck" aria-label="主要ルーム記録">${honors.map((h,i)=>`<button type="button" data-action="focus-honor" data-honor-index="${i}" class="honor-card card-${i+1} ${h.red?'red-honor':''} ${activeHonorIndex===i?'is-active':''}" aria-pressed="${activeHonorIndex===i?'true':'false'}"><div class="honor-corner">${h.corner}</div><div class="honor-label">${h.label}</div><div class="honor-value">${h.value}</div><div class="honor-divider"></div><div class="honor-holder">${esc(h.name)}</div><div class="honor-date">${h.date}</div><div class="honor-tag">${h.tag}</div><div class="honor-corner bottom">${h.corner}</div></button>`).join('')}</section>
      <section class="honor-detail ${active.red?'red-detail':''}">
        <div class="honor-detail-title"><span>${active.corner.slice(-1)}</span><strong>${active.label}</strong></div>
        <p>${active.description}</p>
        <div class="honor-detail-stats"><b>${active.value}</b><strong>${esc(active.name)}</strong><small>${active.date}</small></div>
        <div class="honor-detail-footer"><span>HOUSE HONOR ${String(activeHonorIndex+1).padStart(2,'0')}</span><span>♛</span></div>
      </section>
      <p class="honor-hint">ⓘ カードをタップすると、前面に表示されます</p>
  ...
  function menuModal(){ modal(`<div class="modal-head"><div><div class="eyebrow">SYSTEM · v1.4</div><h2>DATA BOOK</h2></div><button class="icon-btn" data-action="close-modal">×</button></div><div class="menu-stack"><button class="quiet-action wide" data-action="export-data">EXPORT BACKUP</button><label class="quiet-action wide file-label">IMPORT BACKUP<input id="import-file" type="file" accept="application/json"></label><button class="danger-action wide" data-action="reset-data">RESET ALL DATA</button></div><div class="cloud-panel"><span class="cloud-dot ${cloudStatus}"></span><div><b>SUPABASE SYNC</b><small>${cloudStatus.toUpperCase()}</small></div></div><p class="muted copy">Supabaseと同期し、同じURLを開いた複数端末へ変更を反映します。通信できない場合も端末内に保存されます。</p>`); }
  ...
  function exportBackup(){ const blob=new Blob([window.PokerStorage.exportData()],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`poker-rank-v1.4-backup-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000); }
