(function () {
      ...
      <main class="page home-page">
        ...
        <section class="ledger-section standings-section">
          ...
          <div class="standings-list">
            ${st.active.map((pid,i)=>`<div class="standing-row ${i===0?'is-leader':''}"><span class="standing-rank">${String(i+1).padStart(2,'0')}</span><strong class="standing-player"><span class="standing-hand">${handCardsHTML(playerById(pid)?.myHand,{small:true})}</span>${esc(playerById(pid)?.name||'—')}</strong><b>${fmt.format(currentStack(g,pid))}</b></div>`).join('')}
            ${st.out.length?`<div class="out-divider"><span>OUT</span></div>${st.out.map(pid=>`<div class="standing-row is-out"><span class="standing-rank">${g.results[pid].rank?String(g.results[pid].rank).padStart(2,'0'):'—'}</span><strong class="standing-player"><span class="standing-hand">${handCardsHTML(playerById(pid)?.myHand,{small:true})}</span>${esc(playerById(pid)?.name||'—')}</strong><b>OUT</b></div>`).join('')}`:''}
          </div>
        </section>
  ...
  function playersView(){
    ...
    return `${topBar('MEMBERS')}<main class="page players-page">
      <section class="page-intro compact-intro"><div class="eyebrow">PRIVATE MEMBERS CLUB</div><h1>Players</h1><p>マイハンドを紋章にした、ポーカールームのメンバーズボード。</p></section>
  ...
  function graphData(g){
    ...
    nodes.forEach(n=>{
      if(n.type==='break'){
        ...
        const pid=n.event.playerId;if(pid&&active.has(pid)){const seg=segments[pid][segments[pid].length-1];const before=Number(n.event.meta?.stackBefore);if(Number.isFinite(before)&&before>=0&&seg&&seg[seg.length-1]?.t!==n.at)seg.push({t:n.at,v:before});active.delete(pid);delete state[pid];}
      }else if(n.type==='reentry'){
        const pid=n.event.playerId;if(pid){active.add(pid);state[pid]=g.startingStack;theoreticalTotal+=g.startingStack;segments[pid].push([{t:n.at,v:g.startingStack}]);}
      }
  ...
  function renderStackChart(g,{home=false}={}){
    ...
    return `<div class="stack-chart-wrap"><svg class="stack-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="プレイヤー別スタック推移"><g>${grid}${breakMarks}${lines}${avgPath}${xt}</g></svg><div class="chart-legend">${legend}</div><div class="chart-note">横軸はゲーム開始からの実時間。AVERAGEは理論総チップ ÷ ACTIVE人数。OUTで線が切れ、RE-ENTRYは開始スタックから再開します。</div></div>`;
  }
  ...
  function recordsView(){
    ...
    const honors=[
      {corner:'A♠',label:'MOST TITLES',value:titles?.s.titles??'—',name:titles?.p.name||'—',tag:'TITLE HOLDER'},
      {corner:'K♥',label:'BEST PERFORMANCE',value:perf?perf.s.performance.toFixed(1)+'%':'—',name:perf?.p.name||'—',tag:'FIELD MASTER',red:true},
      {corner:'Q♦',label:'MOST GAMES',value:games?.s.games??'—',name:games?.p.name||'—',tag:'IRON PLAYER',red:true},
      {corner:'J♣',label:'RE-ENTRY KING',value:re?.s.reentries??'—',name:re?.p.name||'—',tag:'BACK FOR MORE'}
    ];
    return `${topBar('ROOM RECORDS')}<main class="page records-page">
      <section class="records-opening"><div class="eyebrow">HALL OF RECORDS</div><h1>House<br>Honors</h1><p>このルームの歴史を、4枚の特別なカードに刻む。</p></section>
      <section class="honor-deck" aria-label="主要ルーム記録">${honors.map((h,i)=>`<article class="honor-card card-${i+1} ${h.red?'red-honor':''}"><div class="honor-corner">${h.corner}</div><div class="honor-label">${h.label}</div><div class="honor-value">${h.value}</div><div class="honor-holder">${esc(h.name)}</div><div class="honor-tag">${h.tag}</div><div class="honor-corner bottom">${h.corner}</div></article>`).join('')}</section>
      ...
      <section class="ledger-section additional-records">${sectionHead('SIDE RECORDS')}<div class="mini-record"><span>LONGEST WIN STREAK</span><b>${streak?.streak||0}</b><small>${esc(streak?.p.name||'—')}</small></div><div class="mini-record"><span>CLEANEST PLAYER</span><b>${clean?clean.s.reentryAvg.toFixed(2):'—'}</b><small>${esc(clean?.p.name||'—')} · RE/G</small></div></section>
  ...
  function menuModal(){ modal(`<div class="modal-head"><div><div class="eyebrow">SYSTEM · v1.3</div><h2>DATA BOOK</h2></div><button class="icon-btn" data-action="close-modal">×</button></div><div class="menu-stack"><button class="quiet-action wide" data-action="export-data">EXPORT BACKUP</button><label class="quiet-action wide file-label">IMPORT BACKUP<input id="import-file" type="file" accept="application/json"></label><button class="danger-action wide" data-action="reset-data">RESET ALL DATA</button></div><div class="cloud-panel"><span class="cloud-dot ${cloudStatus}"></span><div><b>SUPABASE SYNC</b><small>${cloudStatus.toUpperCase()}</small></div></div><p class="muted copy">Supabaseと同期し、同じURLを開いた複数端末へ変更を反映します。通信できない場合も端末内に保存されます。</p>`); }
  ...
  function exportBackup(){ const blob=new Blob([window.PokerStorage.exportData()],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`poker-rank-v1.3-backup-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000); }
