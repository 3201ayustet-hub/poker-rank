(function () {
  ...
  function graphData(g){
    ...
    nodes.forEach(n=>{
      if(n.type==='break'){
        ...
        const pid=n.event.playerId;if(pid&&active.has(pid)){const seg=segments[pid][segments[pid].length-1];const before=Number(n.event.meta?.stackBefore);if(seg){const last=seg[seg.length-1];if(Number.isFinite(before)&&before>=0&&last?.t!==n.at)seg.push({t:n.at-1,v:before});seg.push({t:n.at,v:0});}active.delete(pid);delete state[pid];}
  ...
  function renderStackChart(g,{home=false}={}){
    ...
    return `<div class="stack-chart-wrap"><svg class="stack-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="プレイヤー別スタック推移"><g>${grid}${breakMarks}${lines}${avgPath}${xt}</g></svg><div class="chart-legend">${legend}</div><div class="chart-note">横軸はゲーム開始からの実時間。AVERAGEは理論総チップ ÷ ACTIVE人数。OUT時刻に0まで下降し、RE-ENTRYは開始スタックから新しい線で再開します。</div></div>`;
  }
  ...
  function menuModal(){ modal(`<div class="modal-head"><div><div class="eyebrow">SYSTEM · v1.9</div><h2>DATA BOOK</h2></div><button class="icon-btn" data-action="close-modal">×</button></div><div class="menu-stack"><button class="quiet-action wide" data-action="export-data">EXPORT BACKUP</button><label class="quiet-action wide file-label">IMPORT BACKUP<input id="import-file" type="file" accept="application/json"></label><button class="danger-action wide" data-action="reset-data">RESET ALL DATA</button></div><div class="cloud-panel"><span class="cloud-dot ${cloudStatus}"></span><div><b>SUPABASE SYNC</b><small>${cloudStatus.toUpperCase()}</small></div></div><p class="muted copy">Supabaseと同期し、同じURLを開いた複数端末へ変更を反映します。通信できない場合も端末内に保存されます。</p>`); }
